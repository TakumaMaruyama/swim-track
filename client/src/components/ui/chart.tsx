...
const ChartContainer = React.memo(
  ...
  >(({ id, className, children, config, ...props }, ref) => {
    ...
    const baseClassName = "flex aspect-video justify-center text-xs transition-opacity duration-200 [&_.recharts-cartesian-axis-tick_text]:fill-muted-foreground [&_.recharts-cartesian-grid_line[stroke='#ccc']]:stroke-border/50 [&_.recharts-curve.recharts-tooltip-cursor]:stroke-border [&_.recharts-dot[stroke='#fff']]:stroke-transparent [&_.recharts-layer]:outline-none [&_.recharts-polar-grid_[stroke='#ccc']]:stroke-border [&_.recharts-radial-bar-background-sector]:fill-muted [&_.recharts-rectangle.recharts-tooltip-cursor]:fill-muted/10 [&_.recharts-reference-line_[stroke='#ccc']]:stroke-border [&_.recharts-sector[stroke='#fff']]:stroke-transparent [&_.recharts-sector]:outline-none [&_.recharts-surface]:outline-none hover:opacity-95"
...
const ChartTooltipContent = React.memo(
    ...
    (
      ...
      return (
        <div
          ref={ref}
          className={cn(
            "grid min-w-[8rem] items-start gap-2 rounded-xl border border-border/50 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80 px-3 py-2 text-xs shadow-xl animate-fade-in",
            className
          )}
        >
  ...
  (prevProps, nextProps) => {
    ...
    return (
      ...
      prevProps.hideIndicator === nextProps.hideIndicator &&
...
const ChartLegendContent = React.memo(
          ...
          {payload.map((item) => {
            ...
            return (
              <div
                key={item.value}
                className={cn(
                  "flex items-center gap-2 px-2 py-1 rounded-lg hover:bg-accent/50 transition-colors duration-200 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-muted-foreground"
                )}
              >
  ...
  (prevProps, nextProps) => {
    return (
      prevProps.className === nextProps.className &&
      prevProps.hideIcon === nextProps.hideIcon &&
