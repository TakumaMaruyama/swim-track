...
async function main() {
  ...
  if (!['push', 'pull'].includes(action)) {
    console.error('Usage: npx tsx github-sync.ts [push|pull]');
    process.exit(1);
  }
  ...
  try {
    if (action === 'push') {
      ...
      execSync(`git pull "${remoteUrl}" main --allow-unrelated-histories`, {
        cwd: '/home/runner/workspace',
        stdio: 'inherit',
        timeout: 60000,
      });
      console.log('Successfully pulled from https://github.com/TakumaMaruyama/swim-track');
    }
  } catch (err: any) {
